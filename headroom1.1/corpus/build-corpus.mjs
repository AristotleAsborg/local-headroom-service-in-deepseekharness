/**
 * Build the measurement corpus: the shapes that filled a long coding session.
 *
 * Every payload here is SYNTHETIC and generated deterministically by this file —
 * a bulk package README, a directory listing, a test transcript, a JSON
 * manifest. They reproduce the *shapes* that dominate a session's context
 * without reproducing anyone's actual content.
 *
 * That distinction is deliberate. An earlier version of this file embedded a real
 * package README copied out of `node_modules`, which is someone else's
 * copyrighted documentation and must never be published. Generated text of the
 * same shape measures the compressor exactly as well and carries no such
 * problem. Do not reintroduce real payloads here.
 *
 * Every file is written under corpus/ so the benchmark reads bytes from disk
 * instead of reconstructing them.
 *
 * Run with: node corpus/build-corpus.mjs
 */

import { writeFileSync, mkdirSync, readdirSync, statSync } from 'node:fs'
import { join, dirname } from 'node:path'
import { fileURLToPath } from 'node:url'

const HERE = dirname(fileURLToPath(import.meta.url))
mkdirSync(HERE, { recursive: true })

/**
 * A package README of the shape a real one has — front matter, summary, a
 * config sample, a field table, limitations. The prose is generated, not quoted.
 */
function buildReadme() {
  const lines = [
    '---',
    'description: "A local cache bridge for deployments that need a bounded, self-sweeping store in front of a slow backend."',
    'kind: "package-reference"',
    '---',
    '',
    '# @example-org/example-cache-bridge',
    '',
    'English | [中文](README.zh.md)',
    '',
    '## Summary',
    '',
    'This package keeps a bounded working set of recently read records in memory so that repeated reads do not',
    'reach the backend. Entries carry an expiry, a hit counter and a byte cost, and a sweeper evicts whatever',
    'exceeds the configured budget. It is useful in front of a network store, a filesystem index, or any backend',
    'whose latency is dominated by round trips rather than by work. No cache is enabled by default, and the',
    'package owns no storage of its own: every entry lives only as long as the process does.',
    '',
    '## Use this package',
    '',
    'Add the bridge when the same records are read more than once inside one process. One configuration entry is',
    'the whole setup: give the cache a budget and a sweep interval, and the wrapper appears around the backend',
    'client you already use.',
    '',
    '### Minimal configuration',
    '',
    '```yaml',
    '- id: cache-bridge',
    "  name: '@example-org/example-cache-bridge'",
    '  config:',
    '    maxEntries: 1024',
    '    ttlSeconds: 60',
    '    sweepIntervalSeconds: 5',
    '```',
    '',
    '| Field | Default | Meaning |',
    '|---|---|---|',
    '| `maxEntries` | required | Hard ceiling on stored entries |',
    '| `ttlSeconds` | required | How long an entry stays readable |',
    '| `sweepIntervalSeconds` | `5` | How often expired entries are collected |',
    '| `onEvict` | — | Callback invoked with the key and the eviction reason |',
    '| `maxEntryBytes` | `262144` | Entries larger than this are stored by reference |',
    '| `warmOnStart` | `false` | Preload the working set before serving |',
    '',
    '### Key shape and coexistence',
    '',
    'Keys are namespaced by the caller, so two wrappers over the same backend never collide. A key stays stable',
    'while the underlying record keeps its identity, so repeated reads hit the same entry across reloads.',
    '',
    '- Two entries publishing the same key coexist under their own namespaces.',
    '- Two configurations using the same namespace: the later one fails to load.',
    '- A backend that reports the same key twice in one page gets that page treated as inconsistent.',
    '- An update that changes the key shape is rejected rather than silently missing every read.',
    '',
    '## Known Limitations',
    '',
    '- Storage is per process; a restart starts cold and every entry is gone.',
    '- Warm-on-start reads the backend synchronously and will delay startup.',
    '- Eviction is by budget and age, not by usefulness, so a large cold entry can displace a small hot one.',
    '- Metrics are counters only; no histograms or per-key reporting.',
    '',
  ]
  return lines.join('\n')
}

/** A directory listing, the shape that fills many tool results. */
const listing = Array.from({ length: 210 }, (_, index) => {
  const name = 'pkg-' + ['agent', 'session', 'tool', 'client', 'server', 'storage'][index % 6] + '-' + index
  const bytes = 2000 + ((index * 7919) % 90000)
  return '/opt/runtime/node_modules/@example-org/' + name + '   ' + bytes
}).join('\n')

/** A test transcript, the shape of every verification run. */
const transcript = Array.from({ length: 260 }, (_, index) => {
  const suites = ['engine.test.mjs', 'tool-contract.test.mjs', 'verify-host.mjs', 'compression-bench.mjs']
  const ok = (index * 13) % 17 !== 0
  return (ok ? '  PASS  ' : '  FAIL  ') + suites[index % 4] + ': assertion number ' + index + ' checked the behaviour under test'
}).join('\n')

/** A package-lock style JSON manifest. */
const manifest = {
  name: 'example-runtime',
  lockfileVersion: 3,
  packages: Object.fromEntries(
    Array.from({ length: 90 }, (_, index) => {
      const key = 'node_modules/@example-org/example-module-' + index
      return [key, {
        version: '0.1.5-rc.' + (index % 9),
        resolved: 'https://registry.example.com/@example-org/example-module-' + index + '/-/example-module-' + index + '-0.1.5-rc.' + (index % 9) + '.tgz',
        integrity: 'sha512-' + String(index).repeat(43).slice(0, 86),
        license: 'MIT',
        dependencies: { 'example-util-values': '^0.1.5-rc.2' },
      }]
    }),
  ),
}

const files = {
  'readme.md': buildReadme(),
  'listing.txt': listing,
  'transcript.log': transcript,
  'manifest.json': JSON.stringify(manifest, null, 2),
}

for (const [name, content] of Object.entries(files)) {
  writeFileSync(join(HERE, name), content, 'utf8')
}

console.log('corpus written to ' + HERE)
console.log('all payloads are synthetic and generated by this file.')
let total = 0
for (const name of readdirSync(HERE).filter((entry) => entry !== 'build-corpus.mjs').sort()) {
  const bytes = statSync(join(HERE, name)).size
  total += bytes
  console.log('  ' + name.padEnd(18) + bytes.toLocaleString('en-US').padStart(10) + ' bytes')
}
console.log('  ' + 'TOTAL'.padEnd(18) + total.toLocaleString('en-US').padStart(10) + ' bytes')
