...
修复一个会让**新建会话全部失败**的安装残留：早期版本留下的默认 agent preset 指向了已经被删除的
preset。只动了 `install.mjs`，插件本体（`plugin/index.js`、`plugin/lib/engine.js`）与工具契约没有
任何改动。

### 症状

Web 界面里新建对话、选中一个工作区之后，选择**立刻弹回未选择状态**，对话建不起来；重试同样失败。

### 根因

首版除了 host row 之外还装过一个同名 agent preset，后来退役了——preset 在自身 scope 注册工具、
host row 在全局注册，`tools.register()` 对重名直接抛错，选中它会失败。但用户设置文件里那条默认值
还指着它：
...
默认 preset 是**会话未显式选择时的解析目标**。harness 自己的 `AgentPresets.remove()` 删除 preset 时
会顺手清掉指向它的默认值，正是为了避免这种悬空状态；而安装器是直接删目录，没有调用 `remove()`，
于是留下了这一条。这条链路是：
...
- 该段还有别的条目就保留段头，一条都不剩就整段移除（不留 `agent-presets: {}` 这种等价但更含糊的写法）；
- `--dry-run` 只报告不写盘，重复运行不会重复报告，也不改无关的排版。

### 顺带修掉的一处：从别的目录启动时误跳过一轮测试

独立发布包里的 `install.mjs` 用 `node "…\headroom-package\install.mjs"` 这样从别的目录启动时，
`verify.mjs` 会被误判成"不存在"而报 `skip`——那套测试明明在包里。原因是存在性检查按**启动目录**
解析，而子进程按**脚本目录**启动，两者不一致。现在两处都按脚本目录解析，测试不会再被静默跳过。

### 升级与手动修复

跑一次 `node install.mjs`（或用独立发布包 `headroom-package/` 里那一份）即可修好已受影响的部署，
默认 preset 回落到随包默认的 `standard`。**不必重启**：harness 对 `settings.yaml` 有文件监听与热重载。

不想跑安装器的话，手动删掉 `settings.yaml` 里那一行 `default: headroom` 效果相同。

### 这件事的两条教训

- **退役一个东西，要连指向它的引用一起退役。** 删除目录只做了一半，悬空的引用才是故障本身。
- **插件本身没问题，是安装器留下的状态有问题。** 排查时先看部署状态里有没有指向不存在对象的配置，
  再看插件代码。
